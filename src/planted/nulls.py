...
from .model import FAMILY_CANONICAL, INTERVENTION_VARS, S1PlantedModel
from .readouts import hyperparameter_id
from .scoring import (
...
def _copy_tuple_with_source(
    ...
    *,
    ...
    return BaseSourceTuple(
        ...
        source_input={
            "kind": tuple_record.source_input["kind"],
...
def shuffled_pair_is_available(dataset_bundle: S1DatasetBundle) -> bool:
...
def build_shuffled_pair_dataset_bundle(
    ...
    *,
    ...
    for split_name in ("train", "val"):
        ...
        for (template_family, intervention_type), records in sorted(strata.items()):
            rng = random.Random(
                int(
                    _stable_digest(
                        "shuffled_pair",
    ...
    shuffled_records.sort(key=lambda record: (split_by_group[record.metadata.group_id], record.tuple_id))
    return S1DatasetBundle(tuples=tuple(shuffled_records), split_by_group=split_by_group)
...
class NullSpec:
    ...
    def from_config_extras(
        ...
        *,
        ...
        untrained_model_seed = int(
            extras.get("untrained_model_seed", default_untrained_seed)
        )
        return cls(
            null_families=null_families,
            shuffled_pair_seed=shuffled_pair_seed,
            untrained_model_seed=untrained_model_seed,
        )
            ...
            for site_budget in search_spec.site_budgets:
                ...
                for high_level_model_id in search_spec.high_level_model_ids:
                    ...
                    sampled_proposals.sort(
                        key=lambda item: (
                            item[1]["residual_bits"]["val"],
                            _site_groups_key(item[0]),
                        )
                    )
