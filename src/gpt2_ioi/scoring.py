...
from src.planted.readouts import (
...
from src.planted.scoring import SearchSpec, candidate_table_rows

from .data import S3DatasetBundle
from .model import OTHER_TOKEN, GPT2IOILatents, GPT2IOIRun, GPT2IOIModel
...
def _serialize_site_groups(
    ...
    return {var_name: _site_dicts(site_groups[var_name]) for var_name in VARIABLE_ORDER}
...
def _latents_key(latents: GPT2IOILatents) -> tuple[int, int, int, str]:
    return (latents.n1_index, latents.n2_index, latents.r_value, latents.family)


def _label_for_variable(latents: GPT2IOILatents, variable_name: str) -> int:
    if variable_name == "N1":
...
class GroupProposal:
    ...
    def to_dict(self) -> dict[str, object]:
        return {"sites": _site_dicts(self.sites), "val_nll": self.val_nll}
...
class GPT2IOICandidateSearchEngine:
    def __init__(
        self,
        *,
        model: GPT2IOIModel,
        dataset_bundle: S3DatasetBundle,
        ...
        self.output_alphabet_size = len(self.model.name_vocab) + 1
        ...
        self.class_counts = {
            "N1": len(self.model.name_vocab),
            "N2": len(self.model.name_vocab),
            "R": 2,
        }

    def _build_clean_run_cache(self) -> dict[tuple[int, int, int, str], GPT2IOIRun]:
        cache: dict[tuple[int, int, int, str], GPT2IOIRun] = {}
        for tuple_record in self.dataset_bundle.tuples:
            for latents_dict in (
                tuple_record.metadata.latent_base,
                tuple_record.metadata.latent_source,
            ):
                latents = GPT2IOILatents.from_dict(latents_dict)
    ...
    def _tuple_conditioned_latents(
        ...
        return GPT2IOILatents.from_dict(latents_dict)

    def _feature_vector(self, run: GPT2IOIRun, sites: tuple[Site, ...]) -> torch.Tensor:
    ...
    def _fit_variable_readout(
        ...
        *,
        ...
        fit_result = fit_readout(
            ...
            output_dim=self.class_counts[variable_name],
            seed=seed,
    ...
    def _observed_output_token(
        ...
        *,
        ...
        if cached is not None:
            return cached
        base_latents = GPT2IOILatents.from_dict(tuple_record.metadata.latent_base)
        source_latents = GPT2IOILatents.from_dict(tuple_record.metadata.latent_source)
        ...
        observed_token = (
            patched_run.output_token
            if patched_run.output_token in self.model.name_vocab
            else OTHER_TOKEN
        )
        self.patch_output_cache[cache_key] = observed_token
    ...
    def build_candidate_behavior(
        ...
        *,
        ...
        for latent_key, run in self.clean_run_cache.items():
            ...
            for var_name in VARIABLE_ORDER:
                features = self._feature_vector(run, normalized_site_groups[var_name]).unsqueeze(0)
    ...
    def score_candidate_behavior(
        ...
        *,
        ...
        for split_name, tuple_behaviors in behavior.tuple_behaviors_by_split.items():
            ...
            for tuple_behavior in tuple_behaviors:
                ...
                predictions_for_split.append(
                    (tuple_behavior, self.model.name_vocab[predicted_index])
                )
        ...
        epsilon = min(
            ...
            1 - (1 / self.output_alphabet_size) - epsilon_min,
        )
        ...
        for split_name, predictions_for_split in split_predictions.items():
            ...
            for tuple_behavior, predicted_token in predictions_for_split:
                if predicted_token == tuple_behavior.observed_output_token:
                    ...
                    residual_bits = -math.log2(epsilon / (self.output_alphabet_size - 1))
    ...
    def _score_cell(
        ...
        *,
        ...
        best_site_groups, _, best_record = full_proposal_scores[0]
