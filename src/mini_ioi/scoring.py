...
from .model import NAME_VOCAB, OTHER_TOKEN, MiniIOILatents, MiniIOIRun, MiniIOITransformer
...
def _latents_key(latents: MiniIOILatents) -> tuple[int, int, int, str]:
    return (latents.n1_index, latents.n2_index, latents.r_value, latents.family)


def _label_for_variable(latents: MiniIOILatents, variable_name: str) -> int:
    if variable_name == "N1":
...
class MiniIOICandidateSearchEngine:
    def __init__(
        self,
        *,
    ...
    def _build_clean_run_cache(self) -> dict[tuple[int, int, int, str], MiniIOIRun]:
        cache: dict[tuple[int, int, int, str], MiniIOIRun] = {}
        for tuple_record in self.dataset_bundle.tuples:
            for latents_dict in (
                tuple_record.metadata.latent_base,
                tuple_record.metadata.latent_source,
            ):
                latents = MiniIOILatents.from_dict(latents_dict)
    ...
    def _tuple_conditioned_latents(
        ...
        return MiniIOILatents.from_dict(latents_dict)

    def _feature_vector(self, run: MiniIOIRun, sites: tuple[Site, ...]) -> torch.Tensor:
        ...
        *,
        ...
        for split_name, tuple_behaviors in behavior.tuple_behaviors_by_split.items():
            ...
            for tuple_behavior in tuple_behaviors:
                ...
                predictions_for_split.append((tuple_behavior, NAME_VOCAB[predicted_index]))
