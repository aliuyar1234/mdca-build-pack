...
def _find_worked_example(*, model: GPT2IOIModel, dataset_bundle: Any) -> dict[str, Any]:
    candidate_sites = [
        Site(layer_index=model.n_layers, token_index=model.sequence_length - 1),
        Site(layer_index=model.n_layers - 1, token_index=5),
        Site(layer_index=model.n_layers // 2, token_index=1),
        Site(layer_index=model.n_layers // 2, token_index=3),
    ]
        ...
        for site in candidate_sites:
            ...
            if patched.output_token != base_clean.output_token:
                return {
                    ...
                    "base_prompt_text": base_clean.prompt_text,
                    "source_prompt_text": source_clean.prompt_text,
                }
    tuple_record = next(iter(dataset_bundle.tuples))
    base_latents = GPT2IOILatents.from_dict(tuple_record.metadata.latent_base)
    source_latents = GPT2IOILatents.from_dict(tuple_record.metadata.latent_source)
    fallback_site = Site(layer_index=model.n_layers, token_index=model.sequence_length - 1)
    patched = model.patch_and_run(
        base_latents=base_latents,
        source_latents=source_latents,
        intervention_type=tuple_record.intervention_type,
        patch_sites=(fallback_site,),
    )
    return {
        "split": dataset_bundle.split_by_group[tuple_record.metadata.group_id],
        "tuple_record": tuple_record.to_dict(),
        "candidate_patch_sites": [fallback_site.to_dict()],
        ...
        "patched_output_token": patched.output_token,
        "base_prompt_text": model.run_clean(base_latents).prompt_text,
        "source_prompt_text": model.run_clean(source_latents).prompt_text,
    }


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m src.experiments.gpt2_ioi")
    parser.add_argument("--config", required=True)
    args = parser.parse_args(argv)

    config = load_run_config(args.config)
    if config.setting_id != "gpt2_ioi":
        raise ValueError(
            "python -m src.experiments.gpt2_ioi requires a gpt2_ioi setting config"
        )
    ...
    _json_dump(model.validation_summary, run_dir / "tokenizer_validation.json")
    _json_dump(
        {
    ...
    summary = {
        "status": "ok",
        "entrypoint": "python -m src.experiments.gpt2_ioi",
        ...
        "tokenizer_validation_path": str(run_dir / "tokenizer_validation.json"),
        ...
        "fixed_prompt_length": model.sequence_length,
    ...
    if phase == "smoke":
        ...
        return 0
    if phase != "full":
        raise ValueError(f"Unsupported gpt2_ioi phase: {phase!r}")

    search_spec = SearchSpec.from_config_extras(config.extras)
    search_engine = GPT2IOICandidateSearchEngine(
        model=model,
    ...
    if "untrained_model" in null_spec.null_families:
        untrained_model = GPT2IOIModel(
            device=config.runtime.device,
            dtype=_dtype_from_config(config),
            pretrained=False,
            model_seed=null_spec.untrained_model_seed,
            name_vocab=model.name_vocab,
        )
        records, logs = run_candidate_like_null_search(
